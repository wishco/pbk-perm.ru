...
const Modal = ({
                 dataModalPos = "center", title, active, hideModalWindow, children, modalWindowBusy, modalWindowBusyText, ...data
}) => {
  ...
  useEffect(() => {
    animationObj.initAnimation([modalRef, contentRef, wrapperRef, wrapperAnimRef])
  }, [])
  ...
  const modalKeyTabHandler = (e) => {
    ...
    if ((e.key === KEY_TAB) && (e.shiftKey === true) && (e.target.getAttribute("data-tab-lock") === "start")) {
      e.preventDefault()
    } else if ((e.key === KEY_TAB) && (e.shiftKey === false) && (e.target.getAttribute("data-tab-lock") === "end")) {
      e.preventDefault()
    }
  }
      ...
      >
        ...
        >
          ...
          <div className={s.content}
            ...
            {modalWindowBusyText && <p className="preloader-text-5">{modalWindowBusyText}</p>}




            <div className={s.windowHeader} onClick={(e) => {
