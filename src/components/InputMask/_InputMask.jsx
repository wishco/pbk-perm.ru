...
const _InputMask = ({
  ...
  function getStructureChanger({
    ...
    if (selectionStart_before_calc > selectionEnd_user_calc) {
      // если удаляем символ и смещаемся влево, т.е. нажимаем Backspace
      ...
      return {
        formTel: {
          value:
            value_user.slice(0, selectionStart_before) +
            value_user.slice(selectionStart_before, value_user.length),
          valueCalc:
            value_user.slice(0, selectionStart_before) +
            value_user.slice(selectionStart_before, value_user.length),
    ...
    const partRight = value_user.slice(
      selectionEnd_user_calc,
      value_user.length
    );
    const partMiddle = value_user.slice(
      selectionStart_before_calc,
      selectionEnd_user_calc
    );
    let newPartMiddle = "";
    let newValue = "";
    ...
    if (currMask.regExp === "") {
      //  если тип инпута, ввод только чисел
