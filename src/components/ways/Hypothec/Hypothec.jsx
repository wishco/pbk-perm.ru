...
const Hypothec = ({formHypothec, updateFormHypothec, ...data}) => {
  ...
  const mortgage = (s, p, n) => { // сколько нужно платить в месяц, за кредит
  ...
  const getMortgage = () => {
    ...
    const _cost = getNumFomValue(cost) - (getNumFomValue(cost) * getNumFomValue(pay)) / 100
    ...
    const out = objBox.map((_item, index) => {
      return (
        <div key={index} className={s.containerInput} title={_item.title}>
          ...
          }} className={s.labelInput}>{getLabel(_item.label)}</label>
          <div className={s.boxInput}>
            <div className={s.wrapInput}>
              <input ref={_item.ref} type="text" maxLength={_item.maxLength} className={s.input} tabIndex={1}
                     value={_item.value}
                     onKeyPress={e => {
                       if (e.key.replace(/[^\d]/g, '').length === 0) e.preventDefault()
                     }}
                     onChange={e => changeInputValueIntegerHandler(e, _item.setValue)}/>
            </div>
            <p className={s.unitInput}
               onClick={() => {
                 setFocusInputByRef(_item.ref)
               }}>{_item.unit}</p>
          </div>
      ...
      <div className={s.container}>
        ...
        <div className={s.resultContainer} data-result-active={showResult}>
          <div className={s.resultWrapper}>
            ...
            <p className={s.valCalc}>{cost_result}<span>руб.</span></p>
            <div className={s.itemResultWrapper}>
              <div className={s.itemResultBox}>
                <p className={s.textCalc}>Срок ипотеки</p>
                <p className={s.valCalc}>{time_result}<span>{timeUnit_result}</span></p>
              </div>
              <div className={s.itemResultBox}>
                <p className={s.textCalc}>Первый взнос</p>
                <p className={s.valCalc}>{pay_result}<span>%</span></p>
              </div>
              <div className={s.itemResultBox}>
                <p className={s.textCalc}>Процент в год</p>
                <p className={s.valCalc}>{rate_result}<span>%</span></p>
              </div>
            </div>

            <p className={s.textCalc}>Оплата по</p>
            <p className={s.valCalc}>{payMonth}<span>руб.мес.</span></p>
          </div>
        </div>
