...
const PlaceToServer = ({ placesStructureStandard, ...props }) => {
  ...
  async function txtHandler(e) {
    const structureFromClipboard = await getStructureFromClipboard(
      placesStructureStandard
    );
    if (structureFromClipboard) {
      // если структура соответствует в буфере обмена,
    ...
    <>
      <div>
        ...
        <Wrapper>
          ...
          />
          <button
            className={[s.submit, txtVal && s.goodStructure].join(" ")}
        ...
        <Wrapper>
          <p
            className={[
              s.sendStatusOff,
              sendStatus === SEND_ERROR && s.sendStatusError,
            ].join(" ")}
          >
            Данные не отправлены на сервер, пароль не верен...
          </p>
          <p
            className={[
              s.sendStatusOff,
              sendStatus === SEND_GOOD && s.sendStatusGood,
            ].join(" ")}
          >
