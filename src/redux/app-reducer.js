import {getAllPlacesFromServer, getArrayPlaces, getDataFromUrl} from "api/api";
...
let initialState = {
  ...
  buildOptions: { // опции зданий
    [constBuilds.SNESH_SKAZKA]: { // nameBuild - имя здания
        typesRoomTypicalFilter: ["П1","П2-","П2s-","П3"], // тип комнаты (1-однокомн, 2-двухкомн, 3-трехкомн, 2s-евродвушка
        typesRoomTypicalAll: [
          {type:"П1", text1:"Однокомнатные", text2:"от 1,8 млн.руб.", text3:"площадь от 35 кв.м."},
          {type:"П2-", text1:"Двухкомнатные", text2:"от 2 млн.руб.", text3:"площадь от 45 кв.м."},
          {type:"П2s-", text1:"Евродвушки", text2:"от 2.2 млн.руб.", text3:"площадь от 55 кв.м."},
          {type:"П3", text1:"Трёхкомнатные", text2:"от 3 млн.руб.", text3:"площадь от 65 кв.м."}
        ],
  ...
  placesStructureStandard:{
    ...
    payPartForRoom: {column: constPlaces.payPartForRoom, pattern: "стоимость в рассрочку"}, // стоимость в рассрочку
...
const appReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case CHANGE_FIELD_OF_BUILD_SELECT:
      let _out = {...state.buildSelect[action.buildFieldChange.nameBuild], [action.buildFieldChange.fieldName]: action.buildFieldChange.fieldValue}
      let _isEqualNowAndLast = state.buildSelect[action.buildFieldChange.nameBuild][action.buildFieldChange.fieldName] === action.buildFieldChange.fieldValue

      if (_isEqualNowAndLast && (!action.buildFieldChange.toggleOff)) {
        _out = {...state.buildSelect[action.buildFieldChange.nameBuild],
          [action.buildFieldChange.fieldName]: null}
      } // если старое значение равно новому И toggle не отключен, то снимаем выбор

      return {
        ...state,
        buildSelect: {...state.buildSelect, [action.buildFieldChange.nameBuild]: _out}
      }
...
export const initializeApp = () => (dispatch) => {
  ...
  getArrayPlaces().then((res) => { // получить объект из мест, каждое место содержит массив данных {xxx1: [[...,...]]}...}
