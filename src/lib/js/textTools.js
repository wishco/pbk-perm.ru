...
export function getArrayFromText(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    // последовательный перебор всех символов из строки
...
export function getStructureFromArray(array, placesStructureStandard) {
  ...
  for (let i = 0; i < objVals.length; i++) {
    ...
    if (
      // проверяем заголовок текста на соответствие структуре, заголовок из двух строк
      isExistText({
        cellText: array[ROW_HEAD][objVerif.column],
        pattern: objVals[i].pattern,
      }) || // если строка текста(ячейка) содержит, необходимый текст // или
      isExistText({
        cellText: array[ROW_HEAD + 1][objVerif.column],
        pattern: objVals[i].pattern,
      }) // если строка текста(ячейка) содержит, необходимый текст [расположена на ряд ниже]
    ) {
      ...
      structureBad[objKeys[i]].cellTextRowPlusOne =
        array[ROW_HEAD + 1][objVerif.column];
      structureBad[objKeys[i]].pattern = objVals[i].pattern;
    }
  }

  if (
    Object.keys(structureNorm).length !==
    Object.keys(placesStructureStandard).length
  ) {
    const styleConsoleLog = "background: pink; color: black";
    console.log(
      "%c Текст буфера обмена, не соответствует требуемой структуре, проблема в поле:",
      styleConsoleLog
    );
...
export async function getStructureFromClipboard(placesStructureStandard) {
  ...
  const structurePlace = getStructureFromArray(
    arrayFromText,
    placesStructureStandard
  ); // получить соответствие таблицы, смотрим шапку и соответствие колонок шапки (структура дома - квартиры, этажи, подъезды и прочее)

  if (!structurePlace) return ""; // если объект не создан, возвращаем NULL

  return {
    // если объект создан, возвращаем объект
